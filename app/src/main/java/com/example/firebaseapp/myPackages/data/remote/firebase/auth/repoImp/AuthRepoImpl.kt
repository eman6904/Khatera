            ...
            override fun onComplete(p0: Task<AuthResult>) {
                if (p0.isSuccessful) {
                    ...
                    sendEmailVerification(
                        ...
                        onFailer = { onFailer(it) }
                    )

                } else {
                    // email or password is not written correctly
                    onFailer(context.getString(R.string.email_or_password_is_not_written_correctly))
                }
            }
        })
    }

    override fun sendEmailVerification(onSuccess: () -> Unit, onFailer: (String) -> Unit) {
        ...
        authRef?.currentUser?.sendEmailVerification()?.addOnCompleteListener {
            if (it.isSuccessful) {
                ...
                onFailer(context.getString(R.string.something_went_wrong_try_again))
            }
        }
    }
            ...
            password
                ...
                override fun onComplete(p0: Task<AuthResult>) {
                    if (p0.isSuccessful) {
                       ...
                       onFailer(context.getString(R.string.email_or_password_incorrect))
                    }
                }
            })
    }

    override fun verifyEmailAddress(onSuccess: () -> Unit, onFailer: (String) -> Unit) {
        //for verify email
        if(authRef?.currentUser!!.isEmailVerified)
        {
            onSuccess()
        }else{
            onFailer(context.getString(R.string.please_verify_your_account))
        }
    }
}